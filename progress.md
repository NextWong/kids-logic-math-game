...
- Replaced browser-first prompt speech with built-in MP3 prompt audio generated by `generate-prompt-audio.sh`; system speech synthesis is now fallback only.
- Bumped static asset references and service worker cache to `v6` for the built-in prompt audio rollout.
- Reworked phone layout so the canvas is taller and answer buttons are a compact single row on small screens.
...
- Made age difficulty explicit in both code and UI: 3-year-olds avoid advanced logic, 4-year-olds add compare/pairing, 5-year-olds add subtraction and number patterns, and 6-year-olds add missing-addend problems and skip-count patterns.
- Added repeatable animal tap reactions with per-animal action state: jump, wave, dance, spin, peekaboo, and heart effects.
